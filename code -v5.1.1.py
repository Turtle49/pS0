...
# v4.1: continuous improvement with implementation
# v5: ditched baseline in favor of distance to line of ratios G/R and B/R
# v5.1: changes to library to accept other TCS34725 devices (added address 10)
# v5.1.1: cumulative save. relatve/baseline Is_it_color
...
min_threshold = 20 #20 for gain = 16 # Minimum size of signal vectors, its modulus, to be considered signal, not noise
max_threshold = 5000 #200 for gain = 16 # Max size of signal vectors, its modulus, to be considered signal, not noise
min_threshold_RGB = 5 #10 for gain = 16 threshold for **each individual channel**min_threshold_RGB = 10000000
...
#color_ref = [[45,40,41]]  # Sunshine Plastics grayvia Orange
#color_ref = [[100,85,84]]  # Sunshine Plastics grayvia Orange
color_ref = [[40,40,40]]  # Sunshine Plastics gray in slide channel R&D
color_ref_256 = [[025*256 , 025*256 , 025*256]]  # Sunshine Plastics gray in slide channel R&D


#color_ref_ratio = [3225.505, 3627.548, 3450]
 #Gray
#color_ratios = [0.9738, 1.0755, #Red

# Tolerance in color determination
color_error = 0.2  # max angle, in radians, between data and reference vectors
error_ratio_base = .4 #max error between the relative, baseline, values read and reference (using 256 color scaling)


bad_counter_threshold = 0  # max number of readings of "bad" particles before the valve is activated
nothing_counter_threshold = 1  # minimum number of readings of "nothing" before "bad" counter, bad_counter_threshold is reset.

# RGB Digital sensor function
def digitalRGB_sensor(i):
    #return list(sensorRGB[i].color_rgb_bytes)  # For TCS34725
    return list(sensorRGB[i].color_raw)  # For TCS34725
    #return list(sensorRGB[i].color_rgb_bytes)  # For TCS34725 255 scale
    #return list(sensorRGB[i].clear_color_raw)  # For TCS34725
    #return list(sensorRGB[i].color_raw8)  # For TCS34725 (for using H and L bits)

def digitalRGB_sensor_clear(i):
    #return list(sensorRGB[i].color_rgb_bytes)  # For TCS34725
    #return list(sensorRGB[i].color_raw)  # For TCS34725
    return list(sensorRGB[i].clear_color_raw)  # For TCS34725
    #return list(sensorRGB[i].color_raw8)  # For TCS34725 (for using H and L bits)
...
# This function measures the angle between the 'vector' data and multiple reference 'vectors' in the list reference (e.g., RGB_ref) and returns the idex of the lowest value
def is_it_color_ratio_base(data, reference_256, baseline, error):
    #print(data,baseline,reference_256,error)
    errors = []
    for ref_256 in reference_256:
        dEr = data[0] - baseline[0]; dIr = ref_256[0] - baseline[0]
        dEg = data[1] - baseline[1]; dIg = ref_256[1] - baseline[1]
        dEb = data[2] - baseline[2]; dIb = ref_256[2] - baseline[2]
        print('is_it_color_ratio_base',(1-dIg * dEr / dEg / dIr), "--", (1-dIb * dEr / dEb / dIr), '--', ( (1-dIg * dEr / dEg / dIr)**2 + (1-dIb * dEr / dEb / dIr )**2 ) **.5)
        errors.append( ( ( 1 - dIg * dEr / dEg / dIr)**2 + (1 - dIb * dEr / dEb / dIr)**2 ) **.5)
    if error >= min(errors):
        return True
    else:
        return False
...
# This function measures the angle between the 'vector' data and multiple reference 'vectors' in the list reference (e.g., RGB_ref) and returns the idex of the lowest value
#def is_it_color_ratio(data, reference, error):
    #print(data[1]/data[0] - reference[1]/reference[0], "--", data[2]/data[0]- reference[2]/reference[0])
#    if error >= abs(data[1]/data[0] - reference[1]/reference[0]) and abs(error >= data[2]/data[0] - reference[2]/reference[0]):
#        return True
#    else:
#        return False
...
for i, port in enumerate(multi_port):
    ...
    sensorRGB[i] = mrs_tcs34725_v3.TCS34725(tca[port])  ##WARNING: in Trinket M0 the UART object MUST be created before I2C https://learn.adafruit.com/circuitpython-essentials/circuitpython-uart-serial
    #sensorRGB[i] = adafruit_tcs34725.TCS34725(tca[port])  ##WARNING: in Trinket M0 the UART object MUST be created before I2C https://learn.adafruit.com/circuitpython-essentials/circuitpython-uart-serial
...
data_raw_multi_previous = [[0.0, 0.0, 0.0]] * len(multi_port)


#Initialization of good/bad counters
bad_counter = [0, 0, 0] * len(multi_port)
good_counter = [0, 0, 0] * len(multi_port)
nothing_counter = [0, 0, 0] * len(multi_port)
#time_stamp = 0 #timestamp, for debugging

#Initial readings need to be cleared, to avoid garbage being counted
for i in range(4):
    for k in range(len(multi_port)):
        #data_raw_multi[k] = digitalRGB_sensor(k)
        data_raw_multi[k] = [0,0,0]
        time.sleep(0.1)
...
while True:
    ...
    #Tentative pause to wait for values to be fully written in
    time.sleep(RGB_int_time / 2 / 1000 / len(multi_port))
    #Sensors DAQ loop
    for k in range(len(multi_port)):
        # Sensor readout
        ...
        #tic = time.monotonic()
        data_raw_multi[k] = digitalRGB_sensor(k)
        #print(time_stamp - time.monotonic())
        #print(data_raw_multi[k],data_base_multi[k])
        ...
        if distance(data_raw_multi[k], data_base_multi[k]) > min_threshold :#and distance(data_raw_multi[k], data_base_multi[k]) < max_threshold:
            ...
            #print(tuple(data_raw_multi[0]),  sensorRGB[k].clear_color_raw)
            #print(int(sensorRGB[k].clear_color_raw))
            #print(list(sensorRGB[k].color_raw))
            #print(list(digitalRGB_sensor_clear))


            #Check that min threshold is met for each RGB
            if abs(data_multi[k][0]) > min_threshold_RGB and abs(data_multi[k][1]) > min_threshold_RGB and abs(data_multi[k][2]) > min_threshold_RGB:
                ...
                if data_raw_multi[k][0] == data_raw_multi_previous[k][0] or data_raw_multi[k][1] == data_raw_multi_previous[k][1] or data_raw_multi[k][2] == data_raw_multi_previous[k][2]:
                    #print("repeated", data_raw_multi, data_raw_multi_previous, data_raw_multi[k][0] == data_raw_multi_previous[k][0] , data_raw_multi[k][1] == data_raw_multi_previous[k][1] , data_raw_multi[k][2] == data_raw_multi_previous[k][2])
                    pass
                else:
                    #refresh the comparison variable
                    #data_multi_previous[k] = [x1 - x2 for (x1, x2) in zip(data_raw_multi[k], data_base_multi[k])]
                    data_raw_multi_previous[k] = data_raw_multi[k]
                    # Comparison to reference values for identification

                    #Testing alt algorithm: not active, just running it
                    #is_it_color_ratio_base(data_raw_multi[k], color_ref_256, data_base_multi[k], error_ratio_base)

                    if is_it_color(data_multi[k], color_ref, color_error):
                        good_counter[k] = good_counter[k] + 1 ; bad_counter[k] = 0 ; nothing_counter[k] = 0
                        print(time.monotonic(),",", multi_port[k],",", tuple( [x1 - x2 for (x1, x2) in zip( data_raw_multi[k], data_base_multi[k] ) ] ), ",good,", sum(good_counter),",",sum(bad_counter) ,",",sum(nothing_counter) ,",", "graySunshine", )
                        print("Baseline: ", data_base_multi)
                        # particle is not marked for valving
                    else:
                        good_counter[k] = 0 ; bad_counter[k] = bad_counter[k] + 1 ; nothing_counter[k] = 0
                        if sum(bad_counter) > bad_counter_threshold:
                            # Mark time for valving
                            particle_arrival.insert(0, time.monotonic())
                            print(time.monotonic(),",", multi_port[k],",", tuple( [x1 - x2 for (x1, x2) in zip( data_raw_multi[k], data_base_multi[k] ) ] ), ",bad,", sum(good_counter),",",sum(bad_counter) ,",",sum(nothing_counter),",", "graySunshine", )
#                            print(time.monotonic(),",", k+1,",", tuple( data_raw_multi[k]), ",bad,", sum(good_counter),",",sum(bad_counter) ,",",sum(nothing_counter),",", "graySunshine", )
                            #bad_counter = [0, 0, 0] * len(multi_port)
                        else:
                            print(time.monotonic(),",", multi_port[k],",", tuple( [x1 - x2 for (x1, x2) in zip( data_raw_multi[k], data_base_multi[k] ) ] ), ",almost bad,", sum(good_counter),",",sum(bad_counter) ,",",sum(nothing_counter),",", "graySunshine", )
            else:
                print(time.monotonic(),",", multi_port[k],",", tuple( [x1 - x2 for (x1, x2) in zip( data_raw_multi[k], data_base_multi[k] ) ] ), ",min/max,",",",",",",")
                pass
        else:
            # The signal is below min. threshold or above max. threshold. Let's check that's not too high to add to rolling average. If too high, ignore.
            if distance(data_raw_multi[k], data_base_multi[k]) < max_threshold:
                ...
                #pass
            nothing_counter[k] =+ 1
            if sum(nothing_counter) > nothing_counter_threshold:
                    good_counter[k] = 0 ; bad_counter[k] = 0 ; nothing_counter[k] = 0
