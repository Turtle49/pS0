...
for i, port in enumerate(multi_port):
    ...
    time.sleep(0.02)
    sensorRGB[i].integration_time = 10 #614.4/10 #for integration time 2.4 - 614.4 ms. Make sure the TCS34725 library adafruit_tcs34725.py has been modified, line 224
...
switch_baseline = digitalio.DigitalInOut(board.D1); switch_baseline.direction = digitalio.Direction.INPUT; switch_baseline.pull = digitalio.Pull.UP
...
relay = digitalio.DigitalInOut(board.D12); relay.direction = digitalio.Direction.OUTPUT
...
while True:
    ...
    while True in flag_check_multi:
        ...
        for i in range(len(multi_port)):
            ...
            if flag_check_multi[i] == True:
                ...
                if data_raw != data_raw_multi or flag_count_multi[i] > 0:
    ...
    if distance(data) > min_threshold:
        ...
        if not is_it_color(data,color_ref,color_error):
